...
public class LjhController {
	...
	public String meetingReportUpdate(Meeting meeting, Model model) {
		System.out.println("LjhController meetingReportUpdate");
		
		int updateResult = ljhs.updateMeetingReport(meeting);
		
		if (updateResult > 0) return "redirect:prj_meeting_report_read";
		else {
			model.addAttribute("msg", "입력 실패 확인해보세요");
			return "forward:prj_meeting_report_update_2";
		}
	}
